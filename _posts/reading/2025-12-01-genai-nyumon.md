...
正規分布を使用することでデータのばらつき具合を抑えることができる。
...
マルチタスク学習：複数のモデルを用意して同時に学習し、高い性能を実現するテクニック

ここではエンコーダが出力する潜在空間の値から入力画像を再現するデコーダと入力画像のラベル値を予測する分類モデルを同時に学習する

![image]({{site.baseurl}}/images/reading/genai_nyumon/multi_ae.png)

今回のデータでは変分オートエンコーダよりデータのまとまりはあるが、クラスター間の隙間は大きくなっている
