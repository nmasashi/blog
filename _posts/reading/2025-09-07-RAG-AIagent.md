...
- LLM を活用した複雑なワークフローを開発するための python のライブラリ
- ワークフローをグラフ構造としてモデル化する
- ここで言うモデルとはノードとエッジで構成されるグラフ構造のこと
- モデル化することで視覚的に理解しやすい形で設計が可能になる
- State というグラフの状態を保存できる
- ノードで処理を実施して、その結果で state を更新するような形で処理を進める

### 9.3 ハンズオン：Q&A アプリケーション
...
最終的な state はこのようになる。今回は一回でジャッジが True になった
...
```json
{
    ...
    "messages": [
        "生成AI製品エキスパートとしてお答えします。\n\n生成AIとは、人工知能の一分野であり、テキスト、画像、音声、動画などのコンテンツを自動的に生成する技術を指します。これには、自然言語処理（NLP）、コンピュータビジョン、音声合成などの技術が含まれます。生成AIは、ディープラーニングを基盤としており、大量のデータを学習することで、新しいコンテンツを生成する能力を持ちます。\n\n代表的な生成AIのモデルには、OpenAIのGPTシリーズやDALL-E、GoogleのBERT、DeepMindのAlphaFoldなどがあります。これらのモデルは、文章の生成、画像の生成、タンパク質の構造予測など、さまざまな応用が可能です。\n\n生成AIの利点としては、クリエイティブな作業の効率化、パーソナライズされたコンテンツの提供、データ分析の自動化などが挙げられます。一方で、倫理的な問題や誤情報の生成、プライバシーの懸念などの課題も存在します。\n\n生成AIは急速に進化しており、今後も多くの分野での応用が期待されています。最新の技術動向を追い続けることが重要です。"
    ],
    "current_judge": True,
    "judgement_reason": "回答は生成AIについての基本的な情報を網羅しており、技術の概要、代表的なモデル、利点、課題、そして今後の展望についても触れています。内容は正確であり、ユーザーの質問に対して適切に答えています。"
}
```

### 9.4 チェックポイント機能：ステートの永続化と再会

LangGraph にはチェックポイント機能というものがあり、特定の地点のステートを保存できる。

- ステートの永続化: ワークフローの実行状態を保存し、同じステートから再開
- エラー回復: 処理中にエラーが発生した場合、直前のチェックポイントから再開
- デバッグ: ワークフロー実行過程を追跡し、問体の原因を特定

## 第 10 章 要件定義 AI エージェントの開発

実装例なので気が向いたら読む

## 第 11 章 エージェントのデザインパターン

### 11.1 エージェントデザインパターンの概要

#### 目標設定と計画生成を行うデザインパターン

| デザインパターン名               | 概要                                                                                               |
| -------------------------------- | -------------------------------------------------------------------------------------------------- |
| パッシブゴールクリエイター       | ユーザーの入力から具体的な目標を抽出する                                                           |
| プロアクティブゴールクリエイター | ユーザーからの指示以外にもユーザーの周辺環境や状況を利用し、能動的に目標を抽出する                 |
| プロンプト/レスポンス最適化      | 得られたプロンプトやレスポンスに最適化プロセスを追加するパターン                                   |
| シングルパスプランジェネレーター | 抽出された目標を達成するための、一連の行動計画を生成する                                           |
| マルチパスプランジェネレーター   | 目標を達成するための複数の行動計画を生成し、その中から最適な行動を選択するパターン                 |
| ワンショットモデルクエリ         | モデルへの単一の問い合わせで、エージェントの行動計画を生成                                         |
| インクリメンタルモデルクエリ     | モデルへの複数回の問い合わせや、外部システムや人間からの情報収集を通し、段階的に行動計画を生成する |

例えば、「週末の計画を立てて」と依頼した場合、パッシブゴールクリエイターで「旅行プランを立てる」という目標を抽出し、シングルパスプランジェネレーターでこの目標を達成するための具体的な手順を立てる。

#### 推論と確実性の向上を行うデザインパターン

| デザインパターン名       | 概要                                                                      |
| ------------------------ | ------------------------------------------------------------------------- |
| 検索拡張生成 (RAG)       | AI エージェントが持っていない情報を外部環境から取得する                   |
| セルフリフレクション     | AI エージェントが自身の出力を評価し、必要に応じて修正する                 |
| クロスリフレクション     | 異なる視点や基準を持つ別の AI エージェントによる評価を行う                |
| ヒューマンリフレクション | 人間からのフィードバックをもとに評価を行う                                |
| エージェント評価器       | AI エージェントの性能や動作を評価し、その結果をフィードバックするパターン |

#### エージェント間の協調

| デザインパターン名 | 概要                                                                                         |
| ------------------ | -------------------------------------------------------------------------------------------- |
| 投票ベースの協調   | 各エージェントが個別に判断や提案を行い、その結果を投票によって集約し、最終的な意思決定を行う |
| 役割ベースの協調   | 各エージェントに特定の役割を割り当て、それぞれの専門性を生かして共同する                     |
| 議論ベースの協調   | エージェント間で議論や対話を行い、意見交換や討論を通じて合意形成や問題解決を図る             |

#### 入出力制御

| デザインパターン名            | 概要                                                                                                                                 |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| マルチモーダルガードレール    | テキスト、画像、音声など多様な形式の入出力に対して、不適切な入力になっていないか、論理的な基準を満たした出力になっているかを制御する |
| ツール/エージェントレジストリ | AI エージェントが利用可能なツールやサブエージェントを管理し、適切に選択・実行する                                                    |
| エージェントアダプター        | AI エージェントと外部ツールやシステムとの間のインターフェースを提供する                                                              |

### 11.2 18 のエージェントデザインパターン

パッシブゴールクリエイターとプロアクティブゴールクリエイターの違いは、前者が計画の立案のみを行うのに対して、後者が計画を立てつつその計画に関連する事柄も実行する。例えば、「朝六時に起こして」といった場合、後者は起こす時間に合わせコーヒーメーカの電源をいれるなど他の行動を今までのユーザーの行動履歴から推測して勝手に動く

シングルパスプランジェネレーターとマルチパスプランジェネレーターの違いは、前者がプランの要素が直列に並んでいるのに対し、後者は分岐などが発生するパターンで使用される。
