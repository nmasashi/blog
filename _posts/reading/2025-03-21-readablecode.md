...
その関数が何のための関数かを自問する。
そして、その関数内で記述されている処理で無関係の問題を処理するためのコードが記述されている場合は、別関数などにする。

個人的には、無関係の下位問題でも一度しか出現しない処理ならばそのまま書いてもいいのではないかと思う

## 11 章 一度に 1 つのことを

同時に複数のことを実施するようなコードにしない。
タスクは小さく分解し、別の関数などに分ける。

## 12 章 コードに思い込める

内容的にはコードを分割する話と似ていると感じた。

ロジックが複雑なものを分割して関数化し、関数名で何をしているかわかりやすくするようなことが記述されている

## 13 章 短いコードを書く

本当に必要なものを実装する。その機能は必要以上にリッチなものになっていなか？などを確認しながら。例えば、一部の地域しか使用しないシステムに対して、時差に関する機能を実装するなど。

標準ライブラリをうまく活用して、ソースコードを小さくすることを図る

## 14 章 テストと読みやすさ

テストコードの可読性は（プロダクトコードより）高くしなければならない。

- テストの変更が恐ろしくならないように
- プロダクトコードの変更を恐れなくていいように
- 既存テストが壊れても簡単に原因がわかるように

テスト失敗時に表示される内容がわかりやすいものにできそうなら、カスタマイズをする。
不用意に大きい値は使わないようにする（何のテストかわかりにくくなる）。

テストの関数名は説明的な名前になっても OK

```txt
void Test_SortAndFillter_BasicString() {...}
void Test_SortAndFillter_NegativeValues() {...}
```
